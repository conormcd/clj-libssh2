...
(defn exec
  "Execute a command and get the results.
   ...
   session-or-host  Either a valid `clj-libssh2.session.Session` object or a
                    map suitable for handing off to
                    `clj-libssh2.session/with-session`.
   ...
   This returns a map with the following keys and values:
   ...
   :signal  A map with the keys :exit-signal :err-msg and :lang-tag which have
            the values from the similarly-named out parameters of
